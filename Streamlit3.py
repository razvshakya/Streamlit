...
sorted_symbol_unique = sorted(df['Symbol'])
selected_symbol = st.sidebar.selectbox('Symbol',( sorted_symbol_unique))

# Filtering data
#df_selected_sector = df[ (df['Symbol'].isin(selected_symbol)) ]
...
def price_plot(symbol):
  ...
  df['Date'] = df.index
  plt.fill_between(df.Date, df.Close, color='lightblue', alpha=0.3)
